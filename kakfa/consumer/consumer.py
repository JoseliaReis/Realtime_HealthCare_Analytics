...
async def consume_payload(loop):
    ...
    query = await session.prepare_future('INSERT INTO sensorpipeline.sensordata JSON VALUES (?);')
    select = await session.prepare_future('SELECT COUNT(*) sensorpipeline.sensordata;')
...
def handle_success(rows):
    ...
    payload = rows[0]
    try:
        process_payload(payload.id, payload.type, payload.content)
    except Exception:
